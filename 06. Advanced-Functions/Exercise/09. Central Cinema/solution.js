function solve() {
    ...
    archive.nextElementSibling.addEventListener('click', () => archive.innerHTML = null);  // clear the archive
    ...
    function addMovieOnScreen(e) {
        ...
        function addToArchive(btn) {
            ...
            if (Number(ticketsSold.value) || ticketsSold.value === '0') {
                ...
                btn.target.parentNode.parentNode.remove(); // remove a movie from the screen
                Array.from(archive.querySelectorAll('button')).forEach(btn => btn.addEventListener('click', (btn) => btn.target.parentNode.remove())); // remove a movie from the archive
